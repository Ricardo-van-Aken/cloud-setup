...
if ! tofu init; then
    echo "[WARNING] OpenTofu init with local state failed."
    ...
    echo "[INFO] OpenTofu init with local state successful."
fi

echo "[INFO] OpenTofu plan"
tofu plan -out ".tfplan.local" >/dev/null

echo "[INFO] OpenTofu showing plan preview."
...
ACCESS_KEY_LOCAL=$(tofu output -raw bucket_spaces_access_key_local 2>/dev/null || echo "")
SECRET_KEY_LOCAL=$(tofu output -raw bucket_spaces_secret_key_local 2>/dev/null || echo "")
if [[ -n "${ACCESS_KEY_LOCAL}" && -n "${SECRET_KEY_LOCAL}" ]]; then
  ...
  echo "[WARNING] Could not extract remote state bucket Spaces credentials for local usage from OpenTofu outputs." >&2
  exit 1
fi
...
if ! tofu init -backend-config="${SHARED_BACKEND_HCL}" -backend-config="key=${STATE_KEY}" -migrate-state; then
    echo "[WARNING] OpenTofu init with remote state failed."
    exit 1
else
    echo "[INFO] OpenTofu init with remote state successful."
fi
